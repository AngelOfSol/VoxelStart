...
void main()
{
	int x_chunk = int(array_dimensions.y * array_dimensions.z);
	int y_chunk = int(array_dimensions.z);
	float x = gl_VertexID / x_chunk;
	float y = (gl_VertexID % x_chunk) / y_chunk ;
	float z = (gl_VertexID % x_chunk) % y_chunk;
	gl_Position = vec4(x, y, z, 1) + adjust * vec4(direction, 1);
	gNormal = vec4(direction * position, 1);
	gActive = (position != 0) ? 1 : 0;
}
